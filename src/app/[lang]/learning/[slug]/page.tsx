...
export default async function LearningPageDetail({
    ...
    const result = await getLearningPageBySlug(slug, validLang);
    ...
    const relatedPages = (allPages as any[])
        .filter(p => p.language === validLang && p.slug !== slug)
        .slice(0, 5);
        ...
        <main className="min-h-screen bg-[#FDFBF7]">
            ...
            <section className="relative h-[70vh] min-h-[500px] flex items-end pb-20 overflow-hidden">
                ...
                <div className="relative z-10 max-w-5xl mx-auto px-6 w-full">
                    {/* Back Button */}
                    <Link
                        href={`/${validLang === 'en' ? '' : validLang + '/'}learning`}
                    ...
                    >
                        <ArrowLeft size={18} />
                        {t.blogPost?.backToBlog || 'Back to Learning'}
                    </Link>
                ...
                <div className="lg:col-span-3">
                    <div className="sticky top-32">
                        <h3 className="font-serif text-2xl font-bold text-[#0F1E19] mb-6 border-b border-[#0F1E19]/10 pb-4">
                            {t.blogPost?.relatedArticles || 'More Guides'}
                        </h3>
                            ...
                            {relatedPages.length > 0 ? (
                                relatedPages.map((related: any) => {
                                    ...
                                    return (
                                        <Link
                                            key={related.id}
                                            href={`/${validLang === 'en' ? '' : validLang + '/'}learning/${related.slug}`}
                                ...
                                <p className="text-[#0F1E19]/40 text-sm italic">
                                    {t.blogPost?.noRelatedArticles || 'No other guides available'}
                                </p>
                            )}
                        </div>
                    </div>
