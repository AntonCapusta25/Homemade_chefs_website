...
import { Search, PlayCircle, BookOpen, ArrowRight, Star, Clock, ShieldCheck, ClipboardCheck, AlertTriangle, Thermometer, Lock } from 'lucide-react';
...
export default function LearningPage() {
    ...
    // Separate pages with and without videos (use ALL pages for videos, not language-filtered)
    const pagesWithVideos = allPages.filter(p => p.youtube_url);
    const pagesWithoutVideos = pages.filter(p => !p.youtube_url); // Use language-filtered for articles
    ...
    // Get video list (all pages with videos from ANY language)
    const videos = pagesWithVideos.slice(0, 3);

    const categories = [
        { name: t('learning.categoryFoodSafety'), icon: <ShieldCheck />, color: "bg-blue-100 text-blue-700" },
        { name: t('learning.categoryHygiene'), icon: <ClipboardCheck />, color: "bg-green-100 text-green-700" },
        { name: t('learning.categoryAllergens'), icon: <AlertTriangle />, color: "bg-orange-100 text-orange-700" },
        { name: t('learning.categoryTemperatures'), icon: <Thermometer />, color: "bg-red-100 text-red-700" },
        { name: t('learning.categoryCompliance'), icon: "⚖️", color: "bg-purple-100 text-purple-700" },
        { name: t('learning.categoryKitchenOps'), icon: "🔪", color: "bg-yellow-100 text-yellow-700" },
    ];
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-[#FDFBF7] via-[#F9F6F0] to-[#E6DCC3] font-sans text-[#0F1E19]">
            {showGate && <NewsletterGate onClose={() => setShowGate(false)} />}
                ...
                <div className="max-w-7xl mx-auto text-center relative z-10">
                    ...
                    <div className="flex justify-center mb-8">
                        <Link href="#access-form" className="inline-flex items-center gap-3 px-6 py-2.5 rounded-full bg-white/5 border border-white/10 backdrop-blur-md text-white hover:bg-white/10 hover:scale-105 transition-all cursor-pointer group shadow-2xl shadow-[#F47A44]/10">
                            <span className="bg-[#F47A44] text-[#0F1E19] text-xs font-extrabold px-2 py-0.5 rounded-full uppercase tracking-wider">{t('learning.badge')}</span>
                            <span className="font-medium text-sm md:text-base">{t('learning.badgeText')}</span>
            ...
            <section className="max-w-7xl mx-auto px-6 py-16">
                <div className="flex justify-between items-end mb-8">
                    <h2 className="font-serif text-3xl md:text-4xl font-bold">{t('learning.topicsTitle')}</h2>
                </div>
            ...
            <section className="bg-white py-20">
                <div className="max-w-7xl mx-auto px-6">
                    <h2 className="font-serif text-3xl md:text-4xl font-bold mb-12 flex items-center gap-3">
                        <Star className="text-[#F47A44] fill-current" /> {t('learning.essentialGuides')}
                    </h2>
                        ...
                        {featuredArticles.map(article => (
                            <Link key={article.id} href={`/${language}/learning/${article.slug}`} className="group block">
                                ...
                                <div className="flex items-center gap-4 text-sm text-gray-500 mb-3">
                                    ...
                                    <span className="flex items-center gap-1"><BookOpen size={14} /> {t('learning.guide')}</span>
                                </div>
            ...
            <section className="max-w-7xl mx-auto px-6 py-20">
                <h2 className="font-serif text-3xl md:text-4xl font-bold mb-12 flex items-center gap-3">
                    <PlayCircle className="text-[#F47A44] fill-current" /> {t('learning.trainingVideos')}
                </h2>
                ...
                <div className="grid lg:grid-cols-2 gap-12">
                    ...
                    {(() => {
                        ...
                        if (videos.length === 0) {
                            return <div className="text-gray-500 p-8">No videos available</div>;
                        }

                        return (
                            <Link
                                href={`/${language}/learning/${pagesWithVideos[0].slug}`}
                            ...
                            >
                                ...
                                {!isSubscribed && (
                                    <div className="absolute top-4 right-4 z-30 bg-black/50 backdrop-blur-sm rounded-full p-3">
                            ...
                            return (
                                <Link key={video.id} href={`/${language}/learning/${video.slug}`} className="flex gap-6 group cursor-pointer hover:bg-white p-4 rounded-2xl transition-colors">
                                    ...
                                    <div className="flex flex-col justify-center">
                                        ...
                                        <span className="text-sm text-gray-500">Video • {t('learning.training')}</span>
                                    </div>
                                </Link >
                            );
                        })}
